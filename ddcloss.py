...
from utils import at_least_epsilon
...
class Contrastive(LossTerm):
    ...
    def __init__(self, cfg):
        ...
        if cfg.negative_samples_ratio == -1:
            ...
            self._mi_loss_forward = self._mi_loss_forward
    ...
    # 特征互信息
    def _mi_compute_joint(self, z1, z2):
        ...
        return p_i_j
    # 特征互信息
    def _mi_loss(self, z1, z2):
        """Contrastive loss for maximizing the consistency"""
        _, k = z1.size()
        p_i_j = self._mi_compute_joint(z1, z2)
        ...
        return loss
    # 特征互信息
    def _mi_loss_forward(self, net, cfg):
        losses = {}
        n_views = len(net.backbone_outputs)
        for u in range(n_views - 1):
            for v in range(u, n_views):
                losses[f"{u}{v}"] = self._mi_loss(net.backbone_outputs[u], net.backbone_outputs[v])
        return losses

    # 聚类互信息
    def _mi_compute_joint_cluster(self, z1, z2):
        """Compute the joint probability matrix P"""
        p_i_j = z1.unsqueeze(2) * z2.unsqueeze(1)
        p_i_j = p_i_j.sum(dim=0)
        p_i_j = (p_i_j + p_i_j.t()) / 2  # symmetrize
    ...
    # 聚类互信息
    def _mi_loss_cluster(self, z1, z2):
        """Contrastive loss for maximizing the consistency"""
        _, k = z1.size()
        p_i_j = self._mi_compute_joint_cluster(z1, z2)
        assert (p_i_j.size() == (k, k))
    ...
    # 聚类互信息
    def _mi_loss_cluster_forward(self, net, cfg):
        losses = {}
        n_views = len(net.backbone_outputs)
        # for u in range(n_views - 1):
        #     for v in range(u, n_views):
        losses["01"] = self._mi_loss_cluster(net.output1, net.output2)
        return losses
    ...
    def loss_mamba(self, net):
        mse = th.nn.MSELoss()
        loss_list = []
        for v in range(2):
            loss_list.append(mse(net.xrs[v], net.backbone_outputs[v].detach()))
        loss = sum(loss_list)
        # print(loss)
        return loss
    ...
    # 对比学习加互信息最大化
    def __call__(self, net, cfg, extra):
        contrastive_loss = self._loss_func(net, cfg, extra)
        mi_loss_dict = self._mi_loss_forward(net, cfg)
        mi_loss = sum(mi_loss_dict.values())

        mi_loss_cluster_dict = self._mi_loss_cluster_forward(net, cfg)
        mi_loss_cluster = sum(mi_loss_cluster_dict.values())

        # 计算mamba编码器的损失loss
        # loss_mamba = self.loss_mamba(net)

        total_loss = (0.3 * contrastive_loss + 0.6 * mi_loss + 0.1 * mi_loss_cluster)
                      # + 0.1 * loss_mamba)
        return total_loss
